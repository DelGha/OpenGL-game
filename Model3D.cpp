...
namespace gps {
	...
	glm::vec3 Model3D::calculateOverallBoundingBoxSize() {
		glm::vec3 minCoords(std::numeric_limits<float>::max());
		glm::vec3 maxCoords(std::numeric_limits<float>::lowest());

		// Loop through all the meshes and aggregate their bounding box sizes
		for (const Mesh& mesh : meshes) {
			glm::vec3 meshBoundingBoxSize = mesh.calculateBoundingBoxSize();

			// Update the minimum and maximum coordinates for the overall bounding box
			minCoords = glm::min(minCoords, -meshBoundingBoxSize * 0.5f);
			maxCoords = glm::max(maxCoords, meshBoundingBoxSize * 0.5f);
		}

		// Calculate the overall bounding box size by subtracting min from max coordinates
